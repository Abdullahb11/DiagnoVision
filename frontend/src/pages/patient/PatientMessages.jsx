...
const PatientMessages = () => {
    ...
    const load = async () => {
      ...
      try {
        ...
        for (const d of relSnap.docs) {
          ...
          const dd = doctorSnap.exists() ? doctorSnap.data() : {}
          rows.push({
            doctorId,
            name: dd.name || 'Doctor',
            specialty: dd.qualification || 'Eye Specialist',
          })
        }
        ...
        msgSnap.docs.forEach((m) => {
          const x = m.data()
          const did = x.doctorId
          if (did && !lastByDoctor[did]) {
            lastByDoctor[did] = {
              text: x.msg || '',
        ...
        if (!cancelled) {
          setConversations(
            rows.map((r) => ({
              ...r,
              lastMessage: lastByDoctor[r.doctorId]?.text || '',
  ...
  useEffect(() => {
    if (!currentUser || !selectedDoctorId) {
      setMessages([])
      return
    }
  ...
  const handleSend = async () => {
    ...
    if (!text || !currentUser || !selectedDoctorId) return
      ...
      >
        <div className="flex items-center gap-4 mb-3 sm:mb-6 shrink-0">
          ...
          <div>
            ...
            <p className="text-dark-400">Chat with your connected doctors</p>
          </div>
        </div>
        ...
        <div className="flex-1 glass-card overflow-hidden flex flex-col lg:flex-row min-h-0">
          <div className="w-full lg:w-64 xl:w-80 border-r-0 lg:border-r border-white/5 border-b lg:border-b-0 flex flex-col flex-shrink-0 max-h-[28vh] max-[1010px]:max-h-[26vh] lg:max-h-none">
            ...
            <div className="flex-1 overflow-y-auto">
              {loadingList ? (
                ...
                <p className="p-4 text-sm text-dark-400">
                  No doctors yet. Connect with a doctor from Find Doctors first.
                </p>
              ) : (
                conversations.map((conv) => (
                  <div
                    key={conv.doctorId}
                    onClick={() => setSelectedDoctorId(conv.doctorId)}
                    className={`p-4 cursor-pointer transition-all duration-200 ${
                      selectedDoctorId === conv.doctorId
                ...
                <div className="p-4 border-b border-white/5 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-dark-400">{selectedConv.specialty}</p>
                    </div>
                  </div>
